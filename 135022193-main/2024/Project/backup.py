...
def index():
    ...
    # If refresh button is pressed refresh the data in the table
    if request.method == "POST":
        if "refreshButton" in request.form:
            ...
            for userProduct in userProducts:
                ...
                URL = db.execute("SELECT URL FROM products WHERE id = ?", productID)
                URL = URL[0]["URL"]

                # Get updates values using scraper and update database values
                dictValues = bol_scraper(URL)
                db.execute("UPDATE products SET ogPrice = ?, currentPrice = ? WHERE id = ?",
                            float(dictValues["ogPrice"]), float(dictValues["currentPrice"]), productID)
